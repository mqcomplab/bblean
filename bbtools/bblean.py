...
def tanimoto_sim_packed(arr, vec):
...
def calc_centroid(linear_sum, n_samples, *, pack):
    """Calculates centroid
...
class _BFSubcluster:
    ...
    def __init__(
        ...
        self.n_features = n_features
        # NOTE: _buffer holds both "linear_sum" and "n_samples"
        ...
        #
        # To modify _buffer, use set|add_to_n_samples_and_linear_sum(...)
        if buffer is not None:
            if linear_sum is not None:
                raise ValueError("'linear_sum' and 'buffer' are mutually exclusive")
            # Automatically sets centroid
            self.set_buffer(buffer)
            if len(mol_indices) != self._buffer[-1]:
                raise ValueError("Expected len(mol_indices) == n_samples")
        else:
            self._buffer = np.zeros((self.n_features + 1,), dtype=np.uint8)
            if linear_sum is not None:
                # Automatically sets centroid
                if len(mol_indices) != 1:
                    raise ValueError("Expected len(mol_indices) == 1")
                self.set_n_samples_and_linear_sum(1, linear_sum)
            else:
                # For an empty subcluster, manually allocate centroid
    ...
    def linear_sum_(self):
        read_only_view = self._buffer[: self.n_features]
    ...
    def set_buffer(self, buffer):
        self._buffer = buffer
        # Manually calculate the centroid
        self.centroid_ = calc_centroid(self.linear_sum_, self._buffer[-1], pack=True)
    ...
    def set_n_samples_and_linear_sum(self, n_samples, linear_sum):
        ...
        self._buffer[: self.n_features] = linear_sum
        self.centroid_ = calc_centroid(self.linear_sum_, n_samples, pack=True)
    ...
    def add_to_n_samples_and_linear_sum(self, n_samples, linear_sum):
        ...
        self._buffer[: self.n_features] += linear_sum
        self.centroid_ = calc_centroid(self.linear_sum_, new_n_samples, pack=True)
    ...
    def merge_subcluster(self, nominee_cluster, threshold):
        ...
        dtype = min_safe_uint(new_n)
        new_ls = np.add(self.linear_sum_, nominee_cluster.linear_sum_, dtype=dtype)
        ...
        if merge_accept(threshold, new_ls, new_n, old_ls, nom_ls, old_n, nom_n):
            self.set_n_samples_and_linear_sum(new_n, new_ls)
