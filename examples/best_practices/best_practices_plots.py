...
def sankey_cluster_flow(clusters_before,
                        clusters_after,
                        min_flow=1,
                        top_n=20,
                        max_flow=None,
    ...
    if show_counts:
        bef_lab = [
            f"before:{i} ({len(clusters_before[i])})"
            for i in before_ids
        ]
        aft_lab = [
            f"after:{j} ({len(clusters_after[j])})"
            for j in top_after_ids
        ]
    else:
...
def plot_cluster_refinement(df: pd.DataFrame | None = None,
    ...
    ax2.set_ylabel('Singleton/Cluster Ratio',
                   color='green',
                   fontsize=12)
...
def plot_threshold_scan(df: pd.DataFrame,
    ...
    ax3_top = ax3.twiny()
    ax3_top.set_xlim(
        threshold_to_step(ax3.get_xlim()[0]),
        threshold_to_step(ax3.get_xlim()[1]))
...
def plot_distributions_by_fp(pairwise_similarities: dict,
                             save_path: str = None):
    plt.figure(figsize=(8, 6))
    colors = ['blue', 'orange', 'green', 'gray', 'red']
    for fp_type, color in zip(pairwise_similarities.keys(), colors):
        plt.hist(pairwise_similarities[fp_type].flatten(),
                 bins=50,
                 alpha=0.5,
                 label=fp_type,
                 color=color)
        plt.axvline(x=np.mean(pairwise_similarities[fp_type]),
...
def plot_branching_factor_scan(df: pd.DataFrame,
                               fps: np.ndarray | None = None,
    ...
    for i, what in enumerate(what_list):
        ...
        im = sns.heatmap(
            ...
            ax=ax,
            cmap='RdYlBu_r',
            vmin=0, vmax=1 if i // 2 == 1 else None,  # set range 0..1 for bottom row
            cbar=show_cbar,
            cbar_kws=cbar_kws,
            annot=False,
        )
